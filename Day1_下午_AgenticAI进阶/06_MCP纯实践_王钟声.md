...
> "现在绘本生成器有文案、有插图、有 HTML——但如果能把每页文字转成语音，学生就能'听'绘本了。怎么做？——通过 MCP 接入 TTS 语音合成。"
>
> "同理，图片 API 也是通过 MCP 接入的。这节课我们要配两个 MCP：图片生成 + 语音合成。"
...
### Step 3：配置图片 MCP Server（5min）

王钟声投屏，教师跟着操作：

**操作 1**：打开 `.claude/mcp.json`

**操作 2**：加入图片 MCP Server 配置（连接学校的 L20 服务器）：
...
**解说**：
- "`flux-image` 是自己起的名字，Agent 会看到这个名字。"
- "`--url` 指向学校的 L20 服务器——你们不需要装模型，服务器上已经跑着 FLUX.2-klein-9B。"
- "Aquiles-Image 提供的是 OpenAI 兼容接口，所以用标准格式就能调。"

**操作 3**：保存，重启 Claude Code。
...
**操作 1**：打开 `.claude/mcp.json`，在已有的 `flux-image` 旁边加入 TTS Server：
...
**解说**：
- "TTS 跑在本地——每台教师电脑上有自己的 Higgs TTS v3。"
- "`--port 8080` 是 llama.cpp 启动的本地 TTS 服务端口。"
- "不需要 GPU 服务器——你自己的电脑就够了。"

> 注意：课前已预装好 llama.cpp + Higgs TTS v3 BF16 模型，服务已在 8080 端口运行。

**操作 2**：保存，重启 Claude Code。
...
- 尝试让 Agent 用图片编辑功能修改某页插图（"把第 2 页的背景色调暖"）
