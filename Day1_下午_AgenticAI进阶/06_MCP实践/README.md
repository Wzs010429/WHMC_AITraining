...
### Step 3：认识两个 MCP Server 配置（6min）

> "课前已经帮你们配好了两个 MCP Server：图片和 TTS。现在打开盖子看里面。"

王钟声投屏，教师打开 `.claude/mcp.json`，一次性看两个配置块：
...
**解说**（两个一起讲，对比理解）：
- "`flux-image` 指向学校的 L20 服务器——FLUX.2-klein-9B 在上面跑着。你的电脑不需要装图片模型。"
- "`higgs-tts` 跑在你自己的电脑上——`--port 8080` 是 llama.cpp 的本地服务端口。图片走服务器（共享），语音走本地（独享）。"
- "两个配置结构一样：`command` + `args` + `env`。MCP 就是统一了这个格式——换一个 Server 也是同样的写法。"
- "未来想换模型？改 URL 和端口就行——Agent 不用重新学，Skill 不用改。"

**验证连通（含在 6min 内）**：

```
测试图片："帮我生成一张测试图片：一只卡通小猫在草地上看书。"
测试 TTS："把这段文字转成语音：'春风轻轻吹过花园，花儿们都醒了。'"
```

观察右侧日志出现 `[API] flux-image` 和 `[API] higgs-tts`，确认两个 MCP Server 都连通。

> "两个 Server，一份配置，一个标准。这就是 MCP——AI 世界的 USB Type-C。"

> 💡 **Pulse Check**："图片和 TTS 都连通了的举下手？"（5 秒快速确认，确保两个 MCP Server 连通后再进入完整流程）

---

### Step 4：跑完整流程 — 带语音+插图的绘本（12min）
